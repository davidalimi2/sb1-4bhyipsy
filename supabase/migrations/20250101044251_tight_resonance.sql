...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "message_labels_access_policy" ON message_labels;
  DROP POLICY IF EXISTS "message_label_assignments_access_policy" ON message_label_assignments;
END $$;
...
DO $$ 
BEGIN
  CREATE POLICY "message_labels_access_policy_v2"
    ON message_labels FOR ALL
    TO authenticated
    USING (user_id = auth.uid());

  CREATE POLICY "message_label_assignments_access_policy_v2"
