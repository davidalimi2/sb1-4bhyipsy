/*
  ...
  1. New Tables
    ...
    - `discovery_template_questions` for template form questions
    - `discovery_template_categories` for organizing templates
  
  2. Changes
    - Adds template support for discovery requests
    - Adds question management for templates
    - Adds category organization
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Templates read access'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'discovery_templates' 
    AND policyname = 'Templates create access'
  ) THEN
    CREATE POLICY "Templates create access"
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Questions read access'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Categories read access'
  ) THEN
