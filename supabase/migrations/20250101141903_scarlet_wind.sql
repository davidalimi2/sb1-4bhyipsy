...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Public read access to lawyers'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'lawyer_education' 
    AND policyname = 'Public read access to lawyer education'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'lawyer_certifications' 
    AND policyname = 'Public read access to lawyer certifications'
  ) THEN
